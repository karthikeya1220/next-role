 ...
 * back, which is the moment the word changes meaning. Everything else on this
 * page is deliberately plain, so this is the one place with motion.
 *
 * Monospace and lowercase because the thing you are looking at is a local tool
 * you run yourself, and the caret says the same.
 *
 * `group-hover` replays the animation; `prefers-reduced-motion` skips straight
 * to the struck-through end state.
 *
 * This file is duplicated in web/components/logo.tsx on purpose. Sharing it
 * would need a workspace or a published package, which is more machinery than
 * 40 lines of JSX deserves. Change one, consider the other.
 */
